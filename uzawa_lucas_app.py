...
st.markdown("""
This application allows you to explore the dynamics of the Uzawa–Lucas model  
...
gamma = st.sidebar.slider("γ — externality strength", 0.00, 0.6, 0.20, 0.01)
delta = st.sidebar.slider("δ — human-capital technology", 0.01, 0.20, 0.08, 0.01)
rho_minus_lambda = st.sidebar.slider("ρ − λ", 0.00, 0.20, 0.04, 0.01)
lambda_param = st.sidebar.slider("λ — physical-capital depreciation", 0.00, 0.10, 0.01, 0.005)
theta = st.sidebar.slider("θ — inverse elasticity of intertemporal substitution", 0.5, 5.0, 2.0, 0.1)
...
# -------------------------------------------------------
# PLOT 1 — Time paths of k(t), c(t), y(t)
# -------------------------------------------------------
...
Increasing **γ** amplifies output through external effects,  
while higher **δ** speeds up human-capital accumulation.
""")
...
st.markdown("""
**Interpretation:**  
Households choose **u(t)** too high in equilibrium when γ>0,  
meaning they devote *too little time* to human-capital accumulation.
""")


# -------------------------------------------------------
# PLOT 3 — Log paths: equilibrium vs planner
# -------------------------------------------------------
...
st.markdown("""
**Interpretation:**  
The planner internalizes the γ externality → allocates more time to education →  
